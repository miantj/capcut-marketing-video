...
| 原视频已有音乐，又提供新 BGM / 多视频各带 BGM | 整片只留一条主音乐床；优先素材背景音乐，其余 `mute`，禁止双 BGM |
| 原声是人声/环境声 | 不当作 BGM；口播冲突则 mute，配乐另选 `music` 源 |
...
可按钩子、痛点、方案、证据/利益、CTA 组织，每屏一个主信息；未提供的优惠事实不得补造。优先停顿/句尾切镜。有口播时压低 BGM，检查双人声、削波、跳音和句尾截断。
...
BGM **优先用成片素材视频的背景音乐**（`original_audio=music`），不是人声/环境声，也不是「有声轨就当配乐」。整片 **一条** 音乐床：多段不同 BGM 只留一条，其余视频 `audio_action=mute`。独立 BGM 音频填 `audio_kind=music`。用户另给文件再用；缺音乐记待补，不默认塞曲库垫乐。有口播时压低、显式 volume，可 fade_in/fade_out；不足则重复源区间并检查接缝。audio_mix 通过须试听证据。
...
- audio 每段含 asset_id/start/end/source_in/source_out/speed/role/volume，role 为 narration/bgm/sfx，可加 fade_in/fade_out。BGM 规则见上节。aligned 的 provided/native_tts 须用 cue_ids 逐句绑定并覆盖说话区间。
