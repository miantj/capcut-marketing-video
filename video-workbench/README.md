...
文案会发送到火山引擎并按账户规则计费。未配置密钥、鉴权失败、额度不足、网络超时或音频断流时，自动放弃本次口播，按原来的文案估时、字幕和背景音乐流程继续制作，结果中显示回退原因。部分生成的音频不进入视频或下载列表；文案超长和本地制作错误仍需处理。重新制作和试听可能再次产生调用费用。默认音色需要语音合成 2.0 对应权限；其他服务版本需更改资源 ID 并选择匹配音色。
