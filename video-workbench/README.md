...
可复制 `.env.example` 为 `.env`，设置团队访问码和工具路径。访问码为内部共享入口，不是个人权限隔离。AI 选片默认关闭；自动口播默认开启，语音服务不可用时回退；服务器日志不返回 API 密钥。管理员负责备份 `data/queue.sqlite3` 和 `data/jobs/`。点击删除任务并确认后，会永久清理该任务的上传副本、预览、草稿包及记录；其他版本及用户电脑上的原始素材不受影响。上传中请先取消，制作中需等待完成；清理失败可重试删除。
...
默认是**自动模板剪辑（按上传顺序）**，并非已经启用 AI 语义选片。字幕时间直接调用已安装 skill 的 durations_from_chars，当前默认每个非空白字符 0.22 秒，不额外增加每屏最短停留或固定停顿，未启用口播时无配音对齐；启用火山口播后按实际音频时长对齐。三种模板在位置、气泡数量与颜色、入场动画上不同，保留原文，显示时隐藏逗号和句号，按实际字体宽度换行。草稿使用本机真实字体及动画资源；预览读取相同文案、字体、关键词和气泡，动画作近似呈现，未通过原生视听验收。目标电脑需在剪映中预先下载悠然体、放大、波浪弹入和波浪弹出，导入脚本会检查资源是否存在。
...
**剪映原生配音、原生导出、旧字幕消除尚未实现**；已支持火山引擎 AI 口播。预览为 approximate，最终视频请在剪映检查并导出。草稿文件和路径校验不等于原生编辑器兼容验收，尤其不保证手机端兼容。

## 可选 AI 选片

管理员配置 `VIDEO_ENABLE_AI=1`、`VIDEO_AI_API_KEY`、`VIDEO_AI_MODEL` 后重启。用户必须另行勾选关键帧分析授权。仅在选用 AI 模式时发送每段视频的 3 张缩略关键帧和文案到固定 OpenAI Responses API；不发送整段视频或音乐，不读取 Codex 登录凭证。
...
在 `.env` 设置 `VIDEO_TTS_API_KEY`（豆包语音新版控制台的 API Key）及 `VIDEO_TTS_RESOURCE_ID=seed-tts-2.0`，重启工作台。密钥仅在后端读取，不进入任务记录、网页或草稿。

页面默认选择“自动生成口播音频”，也可切换为“不生成口播音频”。选择 VV 或输入其他已开通的音色 ID，并设置语速。可先生成前 20 字试听、下载 WAV；制作视频时逐段合成完整文案，按解码后的实际音频时长对齐字幕和镜头，短字幕合并而不增加静音。口播独立进入草稿和预览，完整 WAV 在任务详情下载。首次打开背景音乐默认 15%，可自行调整；切换口播不会重置音量。提交人、口播开关、音色（含自定义 ID）、语速、画幅、选片方式、风格和背景音乐音量在当前浏览器自动保存，刷新或下次打开时恢复；清除浏览器站点数据会重置。

通过“修改一版”复用已有分段配音：只改风格、画幅、选片或音乐时无需重新合成；修改或插入文案只生成变化的分段。缓存按分段文字、音色、语速和语音资源匹配，并验证音频完整性。新版本保存独立副本，删除原版本不影响它；任务详情显示复用和新生成段数。已有成功任务也可导入旧分段音频；旧音频合并过的短句，修改其中一句会重做该合并段。独立新建任务和试听不共享此缓存。

文案会发送到火山引擎并按账户规则计费。未配置密钥、鉴权失败、额度不足、网络超时或音频断流时，自动放弃本次口播，按原来的文案估时、字幕和背景音乐流程继续制作，结果中显示回退原因。部分生成的音频不进入视频或下载列表，但成功分段保留供下一版复用；文案超长和本地制作错误仍需处理。新生成分段和试听可能产生调用费用。默认音色需要语音合成 2.0 对应权限；其他服务版本需更改资源 ID 并选择匹配音色。

协议参考：https://www.volcengine.com/docs/6561/1598757 。真实 TTS 制作验证：`python tests/smoke.py --tts`（会调用语音服务）。
